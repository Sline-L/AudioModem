...
def args():
    p = argparse.ArgumentParser(description="recover an N2 chirp/training/header/payload/tail-chirp recording")
    ...
    p.add_argument(
        "--tail-training",
        action="store_true",
        help="decode frames that include the same known training block after payload before the gap",
    )
    p.add_argument("--ldpc", action="store_true", help="LDPC-decode payload bits after equalization")
    p.add_argument("--tail-search-seconds", type=float, default=0.5)
    p.add_argument("--out", type=Path, default=Path("runs/n2"))
    return p.parse_args()
...
def recover_payload(payload_eq, header, ldpc_enabled):
    payload_eq = np.asarray(payload_eq).ravel()[: header["payload_mod_symbols"]]
    if ldpc_enabled:
        info_bits = int(header["file_size"]) * 8
        code_meta = ldpc_meta()
        coded_bit_count = int(np.ceil(info_bits / code_meta["ldpc_K"]) * code_meta["ldpc_N"])
        llr = llr_from_symbols(payload_eq, header["mod"])[:coded_bit_count]
        llr = deinterleave_llr(llr)
        decoded_bits, ldpc_diag = ldpc_decode_bits(
            llr,
            info_bits,
        )
        ...
        ldpc_diag = {
            "ldpc_decode_iterations": [],
            "ldpc_decode_iterations_mean": None,
            "ldpc_decode_iterations_max": None,
        }
        body = bytes_from_mod(payload_eq, header["mod"])[: header["file_size"]]
    if len(body) != header["file_size"]:
        raise ValueError("decoded payload is shorter than header file size")
    crc_ok = zlib.crc32(body) == header["file_crc32"]
    return body, ldpc_diag, crc_ok


def truth_header_from_source(source, payload_rows, mod="qpsk", ldpc_enabled=False):
    source_bytes = source.read_bytes()
    if ldpc_enabled:
        truth_bits, _ = ldpc_encode_bits(bits_from_bytes(source_bytes))
    else:
        truth_bits = bits_from_bytes(source_bytes)
    payload_mod_symbols = int(np.ceil(len(truth_bits) / bits_per_symbol(mod)))
    return header_bytes(source, mod, payload_rows, payload_mod_symbols, ldpc_enabled=ldpc_enabled)
...
def ber_diagnostics(
    ...
    ldpc_enabled = bool(header.get("ldpc_enabled", ldpc_enabled)) if header else bool(ldpc_enabled)
    payload_rows = int(payload_rows)
    truth_payload_bits = bits_from_bytes(source_bytes)
    truth_coded_bits = truth_payload_bits
    ldpc_payload_meta = {
        "ldpc_blocks": 0,
        "ldpc_padding_bits": 0,
        "ldpc_info_bits": int(len(truth_payload_bits)),
        "ldpc_coded_bits": int(len(truth_payload_bits)),
    }
    if ldpc_enabled:
        truth_coded_bits, ldpc_payload_meta = ldpc_encode_bits(truth_payload_bits)
    payload_mod_symbols = int(np.ceil(len(truth_coded_bits) / bits_per_symbol(mod)))
    if header:
        payload_mod_symbols = int(header["payload_mod_symbols"])
    ...
    truth_header_bits = bits_from_bytes(
        truth_header_from_source(source, payload_rows, mod, ldpc_enabled=ldpc_enabled)
    )[:HEADER_BITS]
    ...
    rx_bits_interleaved = hard_bits(payload_eq, mod)[: len(truth_coded_bits)]
    if ldpc_enabled:
        coded_rx_bits = deinterleave_bits(rx_bits_interleaved)
        interleaved_truth_bits = truth_coded_bits
        if len(truth_coded_bits):
            interleaved_truth_bits = truth_coded_bits[ldpc_interleaver(len(truth_coded_bits))]
        interleaved_errors, interleaved_bits_count, interleaved_ber = bit_error_count(
            rx_bits_interleaved,
            interleaved_truth_bits,
        )
        coded_errors, coded_bits_count, coded_ber = bit_error_count(coded_rx_bits, truth_coded_bits)
        llr = llr_from_symbols(payload_eq, mod)[: len(truth_coded_bits)]
        llr = deinterleave_llr(llr)
        decoded_bits, ldpc_decode_meta = ldpc_decode_bits(
            llr,
            len(truth_payload_bits),
        )
        payload_bits = decoded_bits[: len(truth_payload_bits)]
    else:
        coded_rx_bits = rx_bits_interleaved
        interleaved_errors, interleaved_bits_count, interleaved_ber = bit_error_count(
            coded_rx_bits,
            truth_coded_bits,
        )
        ...
        payload_bits = coded_rx_bits[: len(truth_payload_bits)]
    ...
    return {
        ...
        "ldpc_enabled": bool(ldpc_enabled),
        **ldpc_meta(),
        **ldpc_payload_meta,
        **ldpc_decode_meta,
        "ber_ldpc_off_bit_errors": int(coded_errors),
        "ber_ldpc_off_bits": int(coded_bits_count),
        "ber_ldpc_off": coded_ber,
        "ber_ldpc_off_interleaved_bit_errors": int(interleaved_errors),
        "ber_ldpc_off_interleaved_bits": int(interleaved_bits_count),
        "ber_ldpc_off_interleaved": interleaved_ber,
        "ber_ldpc_off_deinterleaved_bit_errors": int(coded_errors),
        "ber_ldpc_off_deinterleaved_bits": int(coded_bits_count),
        ...
        "ber_payload_raw_coded_bits": int(coded_bits_count),
...
def run_one(receive, out, a, training, chirp):
    ...
    if header_ok and not error:
        try:
            ...
            payload_ldpc_enabled = bool(header.get("ldpc_enabled", False) or a.ldpc)
            recovered_bytes, ldpc_decode_diag, file_crc_ok = recover_payload(
                payload_eq,
                header,
                payload_ldpc_enabled,
            )
            recovered_name = header["name"]
            (out / recovered_name).write_bytes(recovered_bytes)
            (out / "decoded_payload.bin").write_bytes(recovered_bytes)
            if not file_crc_ok:
                error = "ValueError: file CRC mismatch"
    ...
    metrics = {
        ...
        "ldpc_enabled": bool((header or {}).get("ldpc_enabled", a.ldpc)),
        **ldpc_meta(),
        **ldpc_decode_diag,
    ...
    metrics.update(profile_meta(tail_training=a.tail_training, ldpc_enabled=metrics["ldpc_enabled"]))
    ...
    if metrics.get("ber_available"):
        ...
        if metrics.get("ldpc_enabled"):
            print(
                f"ber_ldpc_off={metrics.get('ber_ldpc_off'):.6f} "
                f"ber_ldpc_on={metrics.get('ber_ldpc_on'):.6f}"
            )
    if error:
