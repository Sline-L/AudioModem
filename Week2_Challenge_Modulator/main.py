...
def f02_modulate_one(payload_path: Path, channel_response: np.ndarray, mapping: str, name_prefix: str) -> np.ndarray:
    ...
    #Step 3: Pad QPSK symbols so every OFDM symbol carries bins 1..511.
    symbol_count = int(np.ceil(qpsk.size / DATA_BINS.size))
    padded = np.zeros(symbol_count * DATA_BINS.size, dtype=np.complex128)
    padded[: qpsk.size] = qpsk
    active_symbols = padded.reshape(symbol_count, DATA_BINS.size)
    ...
    fft_frames[:, DATA_BINS] = active_bins
    fft_frames[:, FFT_SIZE - DATA_BINS] = np.conj(active_bins)
...
def f04_run() -> None:
    ...
    try:
        ...
        for index, payload_path in enumerate(payload_files, start=1):
            samples = f02_modulate_one(payload_path, channel_response, args.mapping, args.name_prefix)
