...
PROFILE = "step4_trimmed_mapped"

# Step 4 trimmed used old N=1024 bins:
# 128-160, 164-169, 172-240, 315-357.
# With N=512 the same physical frequencies map to roughly half those bin
# numbers. Bin 85 is left out because it corresponds to the removed 170-171
# notch in the Step 4 profile.
ACTIVE_BINS = np.r_[
    np.arange(64, 81, dtype=int),
    np.arange(82, 85, dtype=int),
    np.arange(86, 121, dtype=int),
    np.arange(158, 179, dtype=int),
]
PILOT_BINS = np.asarray([64, 72, 80, 82, 84, 86, 96, 106, 120, 158, 166, 174, 178], dtype=int)
...
def profile_meta():
    return {
        ...
        "symbol_len": L,
        "symbol_seconds": L / FS,
        "active_ranges": [[64, 80], [82, 84], [86, 120], [158, 178]],
        "mapped_from_step4_n1024_ranges": [[128, 160], [164, 169], [172, 240], [315, 357]],
