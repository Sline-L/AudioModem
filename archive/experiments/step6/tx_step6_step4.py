...
def args():
    p = argparse.ArgumentParser(description="make the Step 6 N=512, CP=256 Step4-trimmed-band test WAV")
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path("data/step6_newexp/file_combo_step4trimmed_n512_cp256.wav"),
    )
    return p.parse_args()
