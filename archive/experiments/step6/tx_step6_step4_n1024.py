...
from step6_step4_n1024_modem import (
    ACTIVE_BINS,
    FS,
    L,
    MODS,
...
def args():
    p = argparse.ArgumentParser(description="make the Step 6 N=1024, CP=256 Step4-trimmed-band test WAV")
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=Path("data/step6_newexp/file_combo_step4trimmed_n1024_cp256.wav"),
    )
    return p.parse_args()
...
def main():
    ...
    print(
        f"wrote {a.out} seconds={len(raw) / FS:.3f} N=1024 CP=256 "
        f"active_bins={len(ACTIVE_BINS)} mapped_step4_trimmed"
    )
    print(
